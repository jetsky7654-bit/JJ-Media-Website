...
export default async function CasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section case-story-section"><div className="container case-story-grid"><aside className="case-story-nav"><span className="eyebrow">Projektlogik</span><a href="#challenge">01 · Herausforderung</a><a href="#solution">02 · Strategie & Umsetzung</a><a href="#result">03 · Wirkung</a></aside><div className="case-story-content">
      <article id="challenge"><span className="case-chapter">01</span><h2>Die Herausforderung</h2>{item.challenge ? <RichTextContent data={item.challenge}/> : <p>Der Auftritt benötigte mehr Klarheit, Wiedererkennung und eine stärkere Verbindung zwischen Aufmerksamkeit, Expertise und konkretem nächsten Schritt.</p>}</article>
      <article id="solution"><span className="case-chapter">02</span><h2>Strategie & Umsetzung</h2>{item.solution ? <RichTextContent data={item.solution}/> : <p>JJ-Media entwickelte ein modulares System aus Positionierung, wiedererkennbaren Formaten, plattformgerechten Creatives und einer klaren Botschaftshierarchie. Jede Maßnahme erhielt eine eindeutige Aufgabe im Entscheidungsprozess.</p>}</article>
      <article id="result"><span className="case-chapter">03</span><h2>Die Wirkung</h2>{item.result ? <RichTextContent data={item.result}/> : <p>Das Ergebnis ist ein konsistenterer Markenauftritt, eine effizientere Content-Produktion und eine deutlich klarere Verbindung zwischen Sichtbarkeit, Vertrauen und Conversion. Konkrete Leistungswerte werden nur veröffentlicht, wenn sie sauber belegt und freigegeben sind.</p>}</article>
    </div></div></section>
